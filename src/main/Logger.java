...
/**
 * A logger which can store an undefined amount of rows and a defined number of columns. All
 * data should be Numbers. 
 * 
 * Write does one dump of the data into a file specified by the Logger.Config. To add rows,
 * use the addRow method.
 * 
...
public class Logger {
	...
	public static class Config {
		 ...
		 * @param numColumns The number of columns. This must be the same 
		 * 			as the size of the columnNames array
		 * @param filePath The directory where the logs will be stored
		 * @param fileName A name for the log files. This will be the prefix of all the log files.
		 * 			This should be unique to the data you are writing (such as DriveTrajectoryFollower)
		 */
		public Config(String[] columnNames, int numColumns,
	...
	/**
	 * Creates a log file path based on the input config and the log number
	 * argument
	 * @param logNumber
	 * @return an absolute path of a log file
	 */
	public String generatePath(int logNumber) {
		String path = this.config.filePath.getAbsolutePath();
		path = path.concat(
				File.separator + this.config.fileName + logNumber + Logger.EXTENSION);
		return path;
	}
	...
	/**
	 * Returns true if a file has the format of "fileName#.EXTENSION", else
	 * returns false
	 * 
	 * @param file
	 * @return
	 */
	public boolean fileIsLog(File file) {
		String fileName = file.getName();
		
		return fileName.startsWith(this.config.fileName)
				&& fileName.endsWith(Logger.EXTENSION);
	}
	
	/**
	 * Creates a LogFile object from a File object. This associates a File
	 * with a log number. 
	 * @param file The file to convert
	 * @return Null if the file path is not in the correct format, and a LogFile with 
	 * the log number correctly assigned. 
	 */
	public LogFile convertToLogFile(File file) {
		int logNumber = -1;
		String name = file.getName();
		
		String s_num = name.substring(this.config.fileName.length(), name.length() - Logger.EXTENSION.length());
		try {
	 ...
	 * @throws IOException
	 */
	public void toFile(String data, String path) throws IOException {
	...
	public static void main(String[] args) {
		...
		Logger.Config loggerConfig = new Logger.Config(headers, headers.length, 
															dir, fileName);
		...
		try {
			...
			for (int i = 0; i < 1000; i++) {
				logger.addRow(new Number[]
						{i, rand.nextInt(2000), rand.nextInt(2000), rand.nextInt(2000)});
			}
			logger.write();
		} catch (IOException e) {
			e.printStackTrace();
